...
const Navbar: React.FC = () => {
  ...
  const MobileNavbar: React.FC = () => (
    <nav className="fixed top-0 left-0 w-full z-50 rounded-3xl overflow-hidden bg-slate-950/80 h-20 font-Dela text-2xl mt-2 pt-4 font-monomaniacone">
    ...
    <nav className="fixed top-0 left-0 w-full z-50 bg-slate-950/80 h-24 font-Dela">
      <div className="container mx-auto px-6 h-full">
        <div className="flex justify-between items-center h-full">
          <a href="/" className="flex text-white text-4xl transition duration-10000 ease-in-out">
            ドン・クレープ
          </a>
          <button 
            onClick={toggleMenu} 
          ...
          >
            <span className={`block absolute h-1 w-8 bg-white transform transition duration-10000 ease-in-out ${
      ...
      <div className={`fixed top-0 right-0 w-64 h-full bg-slate-950/80 shadow-lg transform transition-transform duration-10000 ease-in-out ${
